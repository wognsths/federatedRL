...
연방 환경에서 오프라인 RL을 실험하는 플레이그라운드다. D4RL(또는 합성) 데이터셋을 비식별(non-IID) 클라이언트 단위로 쪼개고, **FedAvg+SAC** 기본선과 **Fed+OptiDICE** 변형들을 비교한다. 라운드별로 전역 듀얼 `(ν, λ)`을 브로드캐스트하고, 각 클라이언트가 로컬 밀도비 `w_i`로 가중한 정책 학습을 수행한 뒤 다양한 방식으로 파라미터를 집계한다.
...
- 행동 정책의 방문분포를 `\mu_\beta(s, a) = (1-\gamma) \sum_{t\ge0} \gamma^t \Pr_\beta(s_t=s, a_t=a)`라 둘 때, 목표는 새 정책 `\pi_\theta`의 점유비 `\mu_\pi`에 대한 기대 보상 극대화:
  ...
  $$
- 환경 접근 없이 `\mathcal{D}_i`만으로 `w_\pi`를 근사하기 위해 OptiDICE의 듀얼 파라미터 `(ν, λ)`를 학습한다.
...
- 유효 표본 크기(ESS) 기반 가중치:
  ...
  `dual_only_weights=true`이면 `(ν, λ)`에만 `\alpha_i`를, 나머지 파라미터는 `N_i` 비례로 평균한다.
- **λ 정규화**(선택): 전역 평균 비율을 1로 맞추기 위해
...
- **샘플러**: 각 클라이언트는 고정 시드를 가진 미니배치 샘플러를 사용하고, `(1-\gamma)\mathbb{E}_{p_0}[ν]` 항을 위해 에피소드 시작 상태도 함께 샘플한다.
...
- `configs/train_optidice_*.yaml`은 λ 정규화, ESS 가중, 버퍼 가중, 하이브리드 전환 등 논문 실험 대비 세부 변형을 캡슐화한다.
