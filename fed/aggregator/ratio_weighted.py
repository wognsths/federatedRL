...
class RatioWeightedAggregator(Aggregator):
    def __init__(
        self,
        strategy: str = "ess",
    ...
    def _dual_weights(self, updates: List[ClientUpdate]) -> List[float]:
        ...
        return [self._weight_from_metrics(update) for update in updates]

    def _weight_from_metrics(self, update: ClientUpdate) -> float:
        metrics = update.metrics
        mean = metrics.get("ratio_mean")
        std = metrics.get("ratio_std")
        second_moment = metrics.get("ratio_second_moment")
        if mean is None or (std is None and second_moment is None):
            return float(update.num_samples)
        if self.strategy == "ess":
            variance = std ** 2 if std is not None else max(second_moment - mean ** 2, 0.0)
            ess = max(mean ** 2 / (variance + self.eps), self.eps)
            return ess
        variance_inverse = 1.0 / (max(std, self.eps) if std is not None else (second_moment - mean ** 2 + self.eps))
        return variance_inverse
    ...
    def _buffer_mean(self, updates: List[ClientUpdate], weights: List[float]) -> float | None:
        # Use buffer_weights if present; otherwise skip normalization.
        weighted_sum = 0.0
        total = 0.0
